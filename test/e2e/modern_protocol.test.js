...
describe("Modern Protocol E2E", () => {
	...
	before(async () => {
		...
		await fs.writeFile(join(projectPath, "secret.txt"), "The password is BLUE-BIRD");
	...
	it("1. Protocol Sanity: should receive structured JSON turn data", async () => {
		globalThis.fetch = async () => new Response(JSON.stringify({
			model: "mock-model",
			choices: [{ message: { role: "assistant", content: "<response>Hello</response><short>Hi</short>" } }],
		...
		const res = await client.call("init", { projectPath, projectName: "Sanity", clientId: "c1" });
		const result = await client.call("ask", { model: "mock-model", prompt: "Ping" });
		...
		assert.ok(turn.role.system.includes("ASK mode"));
		assert.strictEqual(turn.role.user, "Ping");
		assert.ok(turn.role.assistant.content.includes("<response>Hello</response>"));
	});
	...
	it("2. Multi-turn Loop: should complete an autonomous read-then-summary sequence", async () => {
		...
		const result = await client.call("act", { model: "mock-model", prompt: "Get the secret" });
		...
		assert.ok(turns[1].role.assistant.content.includes("BLUE-BIRD"));
	});

	it("3. Human-in-the-Loop: should pause for prompt_user and resume via run/resolve", async () => {
		globalThis.fetch = async () => new Response(JSON.stringify({
			model: "mock-model",
			choices: [{ message: { role: "assistant", content: "<prompt_user>Color? - [ ] Red - [ ] Blue</prompt_user>" } }],
			usage: { total_tokens: 10 }
		}));

		const result = await client.call("ask", { model: "mock-model", prompt: "What color?" });
		...
		globalThis.fetch = async () => new Response(JSON.stringify({
			model: "mock-model",
			choices: [{ message: { role: "assistant", content: "<response>User chose Red</response><short>Red</short>" } }],
			usage: { total_tokens: 10 }
		}));
