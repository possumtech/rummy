...
describe("Steel Thread E2E", () => {
	...
	before(async () => {
		...
		await fs.writeFile(join(projectPath, "source.txt"), "SECRET_KEY=GOLDEN-TICKET");
	...
	it("ACT Steel Thread: Read source and create result file", async () => {
		...
		globalThis.fetch = async () => {
			...
			if (callCount === 1) {
				...
				content = "<unknown>source.txt</unknown><tasks>- [ ] Read source</tasks><read file=\"source.txt\"/>";
			} else if (callCount === 2) {
				// Turn 1: Model sees content and creates result
				content = "<known>Found GOLDEN-TICKET</known><tasks>- [x] Read source - [ ] Create result</tasks><create file=\"result.txt\">The key is GOLDEN-TICKET</create>";
			} else {
				// Turn 2: Finality
				content = "<tasks>- [x] Create result</tasks><response>Task complete.</response><short>Done</short>";
			}
		...
		await client.call("init", { projectPath, projectName: "SteelThread", clientId: "c1" });
		
		// Run the sequence
		const result = await client.call("act", { model: "mock-model", prompt: "Read source.txt and write the key to result.txt" });
		...
		const createdContent = await fs.readFile(join(projectPath, "result.txt"), "utf8");
