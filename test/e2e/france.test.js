...
describe("Happy Path E2E: France", () => {
	...
	before(async () => {
		...
		execSync('git init && git config user.email "test@test.com" && git config user.name "Test"', { cwd: projectPath });
	...
	it("should answer 'What is the capital of France?' correctly", async () => {
					...
					choices: [
						{
							message: {
								role: "assistant",
								content: "<tasks>- [x] Answer question</tasks><known>The user is asking about France.</known><response>The capital of France is Paris.</response><short>Paris</short>",
							},
						},
					],
		...
		assert.ok(turn0.context.includes("<context"), "Context should be a prettified XML string");
	});
});
