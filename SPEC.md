...
| `log://` | logging | `system, plugin, model, client` | Unified activity tape namespace for all tool actions and prompts. One entry per action at `log://<L>/<T>/<S>/<action>` (`L`=loop sequence, `T`=per-loop turn, `S`=per-turn sequence). |
