...
- [x] **E-15** *(was P-SDI-14)* Duplicate `<rm>` entries in `<previous>` — **fixed**: single-match rm now uses `entry.resultPath` directly (no path construction); multi-match glob deduplicates one path per matched entry and removes the glob-pattern placeholder. `normalizePath` now also lowercases the scheme component. — rm of `unknown://...` created two entries: `rm://turn_7/unknown%3A//...` (dedup collision path) and `rm://unknown%3A//...` (canonical). Root cause: `rm://${match.path}` went through slash-preserving normalization, producing a different encoding than `encodeURIComponent` used by `dedup()`.
...
- [x] **D-2** *(was P-SDI-1)* Prompts tail `<previous>` due to category ordinal — **fixed**: `previous.js` now re-sorts filtered entries by `source_turn` before rendering. Within the same turn, prompts sort before logging (cause before effect). `v_model_context` ordinal unchanged — it governs `<knowns>` KV cache stability, not `<previous>` chronology. — `v_model_context` orders: `tool(1) → data(2) → logging(3) → unknown(4) → prompt(5)`. The prompt that triggered turn 1's `<get>` appears 30+ entries after that `<get>` in `<previous>` at turn 22. Model must reconnect cause to effect across unordered entries.
...
- [~] **D-12** Model writes placeholder `<known>` before having data — **decision: no fix**. Cannot distinguish a placeholder body from a valid short body without semantic understanding. "Mass Effect 1 release year" is 30 chars — a meaningful body is indistinguishable from a question. The 512-token size gate prevents bloat. Model guidance: write knowns only when you have actual data. — in run 2, the model issued `<known>Mass Effect 1 release year</known>` in turn 1 before the search result returned, then a second `<known summary="Mass Effect 1 release year: 2007">Mass Effect 1 was released in 2007</known>` in turn 2. Two entries for the same fact.
