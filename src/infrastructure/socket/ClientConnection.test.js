...
test("ClientConnection", async (t) => {
	...
	const createWs = () => {
		let sent = null;
		return {
			ws: { on: () => {}, send: (d) => { sent = JSON.parse(d); }, readyState: 1 },
			get lastSent() { return sent; }
		};
	};
	...
	await t.test("handleMessage should process ask with buffers", async () => {
		...
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "init", params: { projectPath: "/tmp", projectName: "P", clientId: "c1" }, id: 1 
		})));

		globalThis.fetch = async () => new Response(JSON.stringify({
			choices: [{ message: { role: "assistant", content: "<response>Hi</response>" } }],
			usage: { total_tokens: 5 }
		}));

		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ 
			jsonrpc: "2.0", method: "ask", params: { prompt: "hi", projectBufferFiles: ["a.js"] }, id: 2 
		})));
		
		// The mock WS lastSent getter will work here
		const resp = ws.send.calls ? JSON.parse(ws.send.calls[0]) : {}; // Wait, I need a better way to check sent
	});
	...
	await t.test("handleMessage should process ping", async () => {
		...
		const conn = new ClientConnection(ws, mockDb, createHooks());
		await conn.handleMessageForTest(Buffer.from(JSON.stringify({ jsonrpc: "2.0", method: "ping", id: 10 })));
