...
test("GitProvider", async (t) => {
	await t.test("detectRoot should return null on error", async () => {
		const findRootMock = mock.method(git, "findRoot", () => { throw new Error("no root"); });
	...
	await t.test("getTrackedFiles should return empty set on error", async () => {
		const listFilesMock = mock.method(git, "listFiles", () => { throw new Error("no files"); });
	...
	await t.test("isIgnored should return false on error", async () => {
		const isIgnoredMock = mock.method(git, "isIgnored", () => { throw new Error("error"); });
	...
	await t.test("getHeadHash should return null on error", async () => {
		const resolveRefMock = mock.method(git, "resolveRef", () => { throw new Error("error"); });
